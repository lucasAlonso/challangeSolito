...
export function CoinDetailComp({ data }: { data: CoinsFetchData }) {
  const fill: string = 'rgb(134, 65, 244)'
  ...
  return (
    ...
    >
      ...
      <Text fontSize="lg">
        Market Cap: {data.market_data.market_cap.usd} USD
      </Text>

      <View style={{ height: 600, width: 300, padding: 20 }}>
        <Center>
          <Heading size="md">Variation Percentage</Heading>
        </Center>
        <BarChart
          style={{ height: 200 }}
          data={chartData}
        ...
        <Center m="2" p="5">
          <Heading size="md">Market Cap Change Percentage 24h</Heading>
        </Center>
        <ProgressCircle
          style={{ height: 200 }}
          progress={data.market_data.market_cap_change_percentage_24h / 100}
